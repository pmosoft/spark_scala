...
#define CONFIG_FILE     "./conf/ScheduleDaemon_MultiBand.conf"   // 설정파일명
    ...
    while(s_daemonOnOffFlag) {
        ...
        if(db_testCall() != SQLSUCCESS) {
            ...
            if(ret != SQLSUCCESS) {
                ...
                // 관리자 SMS전송
                sms_sendSystemManager("ScheduleDaemon_MultiBand DB연결 오류 발생. ORA[%d]", ret);
        ...
        // schedule 처리 로직 호출
        ret = procSchedule_MultiBand(&st_server, &st_analysis_weight);
