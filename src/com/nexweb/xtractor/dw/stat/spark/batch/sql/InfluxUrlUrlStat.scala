 ...
 * 출    력 : TB_INFLUX_URL_URL_STAT
 * 수정내역 :
 * 2019-02-07 | 피승현 | 최초작성
 */
object InfluxUrlUrlStat {

  var spark: SparkSession = null
  var objNm  = "TB_INFLUX_URL_URL_STAT"

  var statisDate = ""
  var statisType = ""
  //var objNm  = "TB_INFLUX_URL_URL_STAT";var statisDate = "20190313"; var statisType = "D"
...
/*
          ...
          INSERT INTO TB_INFLUX_URL_URL_STAT(STATIS_DATE, VHOST, VISIT_PATH, INFLUX_DOMAIN, INFLUX_URL, URL, ATTR_ID, VISIT_CNT, TOT_VISITOR_CNT, NEW_VISITOR_CNT, RE_VISITOR_CNT, LOGIN_CNT, LOGIN_VISITOR_CNT, DUR_TIME, PAGE_CNT, PAGE_VIEW, BOUNCE_CNT)
          WITH REFERER_VISIT_PATH AS
          (
          SELECT SESSION_ID, TRIM(CATEGORY) AS VISIT_PATH, HOST AS INFLUX_DOMAIN, DIR_CGI AS CHILD_URL, URL
          ...
          SELECT  ? AS STATIS_DATE,
            ...
            FROM
            (
            SELECT  NVL(VHOST, 'TOTAL') AS VHOST, NVL(TRIM(CATEGORY), 'TOTAL') AS VISIT_PATH, NVL(HOST, 'TOTAL') AS INFLUX_DOMAIN, NVL(DIR_CGI, 'TOTAL') AS CHILD_URL, NVL(URL, 'TOTAL') AS URL, 'ALL' AS ATTR_ID,
            ...
            FROM    TB_WL_REFERER_SEGMENT
            GROUP   BY  ROLLUP ((VHOST, TRIM(CATEGORY), HOST, DIR_CGI, URL))
            ) TA,
            (
            SELECT NVL(VHOST, 'TOTAL') AS VHOST, NVL(VISIT_PATH, 'TOTAL') AS VISIT_PATH, NVL(INFLUX_DOMAIN, 'TOTAL') AS INFLUX_DOMAIN, NVL(CHILD_URL, 'TOTAL') AS CHILD_URL, NVL(URL, 'TOTAL') AS URL, 'ALL' AS ATTR_ID,
              SUM(LOGIN_CNT) AS LOGIN_CNT, COUNT(UNIQUE L_ID) AS LOGIN_VISITOR_CNT
              FROM
              (
              SELECT VHOST, NVL(VISIT_PATH, 'N') AS VISIT_PATH, NVL(INFLUX_DOMAIN, 'N') AS INFLUX_DOMAIN, NVL(CHILD_URL, 'N') AS CHILD_URL, NVL(URL, '/') AS URL, L_ID, LOGIN_CNT
                  FROM TB_MEMBER_CLASS_SEG_STAT TA, REFERER_VISIT_PATH TB
                  WHERE TA.SESSION_ID = TB.SESSION_ID(+)
              )
              GROUP BY ROLLUP ((VHOST, VISIT_PATH, INFLUX_DOMAIN, CHILD_URL, URL))
            ) TB,
            (
            SELECT NVL(VHOST, 'TOTAL') AS VHOST, NVL(VISIT_PATH, 'TOTAL') AS VISIT_PATH, NVL(INFLUX_DOMAIN, 'TOTAL') AS INFLUX_DOMAIN, NVL(CHILD_URL, 'TOTAL') AS CHILD_URL, NVL(URL, 'TOTAL') AS URL, 'ALL' AS ATTR_ID,
              ...
              FROM
              (
              SELECT  VHOST, NVL(VISIT_PATH, 'N') AS VISIT_PATH, NVL(INFLUX_DOMAIN, 'N') AS INFLUX_DOMAIN, NVL(CHILD_URL, 'N') AS CHILD_URL, NVL(URL, '/') AS URL,
                  PAGE_VIEW, DUR_TIME
            ...
            FROM
            (
            SELECT  TA.VHOST, NVL(VISIT_PATH, 'N') AS VISIT_PATH, NVL(INFLUX_DOMAIN, 'N') AS INFLUX_DOMAIN, NVL(CHILD_URL, 'N') AS CHILD_URL, NVL(TC.URL, '/') AS URL, TA.SESSION_ID,
              COUNT(1) BOUNCE_VIEW,
            ...
            GROUP BY V_ID, TA.SESSION_ID, TA.VHOST, VISIT_PATH, NVL(INFLUX_DOMAIN, 'N'), NVL(CHILD_URL, 'N'), NVL(TC.URL, '/')
            )
            GROUP BY ROLLUP ((VHOST, VISIT_PATH, INFLUX_DOMAIN, CHILD_URL, URL))
            ) TD
            WHERE TA.VHOST = TB.VHOST(+) AND TA.VISIT_PATH = TB.VISIT_PATH(+) AND TA.INFLUX_DOMAIN = TB.INFLUX_DOMAIN(+) AND TA.CHILD_URL = TB.CHILD_URL(+) AND TA.URL = TB.URL(+) AND TA.ATTR_ID = TB.ATTR_ID(+)
            AND   TA.VHOST = TC.VHOST(+) AND TA.VISIT_PATH = TC.VISIT_PATH(+) AND TA.INFLUX_DOMAIN = TC.INFLUX_DOMAIN(+) AND TA.CHILD_URL = TC.CHILD_URL(+) AND TA.URL = TC.URL(+) AND TA.ATTR_ID = TC.ATTR_ID(+)
            AND   TA.VHOST = TD.VHOST(+) AND TA.VISIT_PATH = TD.VISIT_PATH(+) AND TA.INFLUX_DOMAIN = TD.INFLUX_DOMAIN(+) AND TA.CHILD_URL = TD.CHILD_URL(+) AND TA.URL = TD.URL(+) AND TA.ATTR_ID = TD.ATTR_ID(+)
 * */
  ...
  def excuteSql() = {
    ...
    sqlDf2.cache.createOrReplaceTempView("REFERER_CATEGORY_HOST_DIR_CGI_URL"); sqlDf2.count()

    qry =
    s"""
      SELECT DISTINCT GVHOST, CATEGORY, HOST, DIR_CGI, URL
      FROM
          (
          SELECT GVHOST
               ...
               , (CASE WHEN URL IS NULL OR URL = '' THEN 'N' ELSE SUBSTR(URL,1,150) END)    AS URL
          FROM   TB_REFERER_SESSION
          UNION
          SELECT GVHOST
               , (CASE WHEN CATEGORY IS NULL OR CATEGORY = '' THEN 'N' ELSE CATEGORY END) AS CATEGORY
    ...
    sqlDf3.cache.createOrReplaceTempView("REFERER_HOST_DIR_CGI_URL_META"); sqlDf3.count()
    ...
    FROM
           (
           SELECT GVHOST, CATEGORY, HOST, DIR_CGI, URL
           FROM REFERER_HOST_DIR_CGI_URL_META
           ) TA
           LEFT OUTER JOIN
           (
           -- 로그인방문자수, 로그인수
           ...
           ON  TA.GVHOST = TB.GVHOST AND TA.CATEGORY = TB.CATEGORY AND TA.HOST = TB.HOST AND TA.DIR_CGI = TB.DIR_CGI AND TA.URL = TB.URL
           LEFT OUTER JOIN
           (
           -- 페이지뷰, 페이지수, 체류시간, 이탈수
           SELECT
           ...
           FROM   TB_ACCESS_SESSION2 TA
                  LEFT OUTER JOIN  REFERER_CATEGORY_HOST_DIR_CGI_URL  TB
           ...
           ON  TA.GVHOST = TC.GVHOST AND TA.CATEGORY = TC.CATEGORY AND TA.HOST = TC.HOST AND TA.DIR_CGI = TC.DIR_CGI AND TA.URL = TC.URL
           LEFT OUTER JOIN
           (
           -- 이탈수
           SELECT
           ...
           ON  TA.GVHOST = TD.GVHOST AND TA.CATEGORY = TD.CATEGORY AND TA.HOST = TD.HOST AND TA.DIR_CGI = TD.DIR_CGI AND TA.URL = TD.URL
           LEFT OUTER JOIN
           (
           -- 방문자수,신규방문자수,재방문자수
           SELECT
           ...
           ON  TA.GVHOST = TE.GVHOST AND TA.CATEGORY = TE.CATEGORY AND TA.HOST = TE.HOST AND TA.DIR_CGI = TE.DIR_CGI AND TA.URL = TE.URL
           LEFT OUTER JOIN
           (
           -- 방문수
           SELECT
           ...
           ON  TA.GVHOST = TF.GVHOST AND TA.CATEGORY = TF.CATEGORY AND TA.HOST = TF.HOST AND TA.DIR_CGI = TF.DIR_CGI AND TA.URL = TF.URL
    GROUP BY TA.GVHOST, TA.CATEGORY, TA.HOST, TA.DIR_CGI, TA.URL
    """
    //spark.sql(qry).take(100).foreach(println);
