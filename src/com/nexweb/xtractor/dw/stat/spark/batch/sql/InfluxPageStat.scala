 ...
 * 출    력 : TB_INFLUX_PAGE_STAT
 * 수정내역 :
 * 2019-02-07 | 피승현 | 최초작성
 */
object InfluxPageStat {

  var spark: SparkSession = null
  var objNm  = "TB_INFLUX_PAGE_STAT"

  var statisDate = ""
  var statisType = ""
  //var objNm  = "TB_INFLUX_PAGE_STAT";var statisDate = "20190312"; var statisType = "D"
...
/*
          ...
          INSERT INTO TB_INFLUX_PAGE_STAT(STATIS_DATE, VHOST, VISIT_PATH, ATTR_ID, URL, VISIT_CNT, TOT_VISITOR_CNT, NEW_VISITOR_CNT, RE_VISITOR_CNT, LOGIN_CNT, LOGIN_VISITOR_CNT, DUR_TIME, PAGE_CNT, PAGE_VIEW, BOUNCE_CNT)
          WITH REFERER_VISIT_PATH AS
          (
          SELECT SESSION_ID, TRIM(CATEGORY) AS VISIT_PATH, URL URL
          ...
          SELECT  ? AS STATIS_DATE,
            ...
            SELECT  NVL(VHOST, 'TOTAL') AS VHOST, NVL(TRIM(CATEGORY), 'TOTAL') AS VISIT_PATH, 'ALL' AS ATTR_ID, URL URL,
            ...
            FROM    TB_WL_REFERER_SEGMENT
            GROUP   BY  ROLLUP (VHOST, TRIM(CATEGORY), URL)
            ) TA,
            (
            -- 로그인 수, 로그인 회원수
            SELECT NVL(VHOST, 'TOTAL') AS VHOST, NVL(VISIT_PATH, 'TOTAL') AS VISIT_PATH, 'ALL' AS ATTR_ID, URL,
              SUM(LOGIN_CNT) AS LOGIN_CNT, COUNT(UNIQUE L_ID) AS LOGIN_VISITOR_CNT
              FROM
              (
              SELECT VHOST, NVL(VISIT_PATH, 'N') AS VISIT_PATH, L_ID, LOGIN_CNT, URL
                  FROM TB_MEMBER_CLASS_SEG_STAT TA, REFERER_VISIT_PATH TB
                  WHERE TA.SESSION_ID = TB.SESSION_ID(+)
              )
              GROUP BY ROLLUP (VHOST, VISIT_PATH, URL)
            ) TB,
            (
            -- 페이지뷰, 체류시간
            SELECT NVL(VHOST, 'TOTAL') AS VHOST, NVL(VISIT_PATH, 'TOTAL') AS VISIT_PATH, 'ALL' AS ATTR_ID, URL,
            ...
            FROM
            (
            SELECT  TA.VHOST, NVL(VISIT_PATH, 'N') AS VISIT_PATH, TA.SESSION_ID, URL,
              COUNT(1) BOUNCE_VIEW,
            ...
            GROUP BY V_ID, TA.SESSION_ID, TA.VHOST, VISIT_PATH, URL
            )
            GROUP BY ROLLUP (VHOST, VISIT_PATH, URL)
            ) TD
            WHERE TA.VHOST = TB.VHOST(+) AND TA.VISIT_PATH = TB.VISIT_PATH(+) AND TA.ATTR_ID = TB.ATTR_ID(+) AND TA.URL = TB.URL(+)
            AND   TA.VHOST = TC.VHOST(+) AND TA.VISIT_PATH = TC.VISIT_PATH(+) AND TA.ATTR_ID = TC.ATTR_ID(+) AND TA.URL = TC.URL(+)
            AND   TA.VHOST = TD.VHOST(+) AND TA.VISIT_PATH = TD.VISIT_PATH(+) AND TA.ATTR_ID = TD.ATTR_ID(+) AND TA.URL = TD.URL(+)
 ...
 * */
  ...
  def excuteSql() = {
    ...
    sqlDf2.cache.createOrReplaceTempView("REFERER_CATEGORY_URL"); sqlDf2.count()
    
    qry =
    s"""
      SELECT DISTINCT GVHOST, CATEGORY, URL
      FROM
          (
          SELECT GVHOST
    ...
    sqlDf3.cache.createOrReplaceTempView("REFERER_URL_META"); sqlDf3.count()
    ...
    FROM
           (
           SELECT GVHOST, CATEGORY, URL
           FROM REFERER_URL_META
           ) TA
           LEFT OUTER JOIN
           (
           -- 로그인방문자수, 로그인수
           ...
           ON  TA.GVHOST = TB.GVHOST AND TA.CATEGORY = TB.CATEGORY AND TA.URL = TB.URL
           LEFT OUTER JOIN
           (
           -- 페이지뷰, 페이지수, 체류시간
           SELECT
           ...
           FROM   TB_ACCESS_SESSION2 TA
                  LEFT OUTER JOIN  REFERER_CATEGORY_URL  TB
                  ON TA.GVHOST = TB.GVHOST AND TA.SESSION_ID = TB.SESSION_ID
           GROUP BY TA.GVHOST, (CASE WHEN TB.CATEGORY IS NULL OR TB.CATEGORY = '' THEN 'N' ELSE TB.CATEGORY END), 
           (CASE WHEN TB.URL IS NULL OR TB.URL = '' THEN 'N' ELSE TB.URL END)
           ) TC
           ON  TA.GVHOST = TC.GVHOST AND TA.CATEGORY = TC.CATEGORY AND TA.URL = TC.URL
           LEFT OUTER JOIN
           (
           -- 이탈수
           SELECT
           ...
           ON  TA.GVHOST = TD.GVHOST AND TA.CATEGORY = TD.CATEGORY AND TA.URL = TD.URL
           LEFT OUTER JOIN
           (
           -- 방문자수,신규방문자수,재방문자수
           SELECT
           ...
           ON  TA.GVHOST = TE.GVHOST AND TA.CATEGORY = TE.CATEGORY AND TA.URL = TE.URL
           LEFT OUTER JOIN
           (
           -- 방문수
           SELECT
           ...
           ON  TA.GVHOST = TF.GVHOST AND TA.CATEGORY = TF.CATEGORY AND TA.URL = TF.URL
    GROUP BY TA.GVHOST, TA.CATEGORY, TA.URL
    """
    //spark.sql(qry).take(100).foreach(println);
