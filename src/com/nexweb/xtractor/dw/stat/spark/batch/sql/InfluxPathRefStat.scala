 ...
 * 출    력 : TB_INFLUX_PATH_REF_STAT
 * 수정내역 :
 * 2019-02-07 | 피승현 | 최초작성
 */
object InfluxPathRefStat {

  var spark: SparkSession = null
  var objNm  = "TB_INFLUX_PATH_REF_STAT"

  var statisDate = ""
  var statisType = ""
  //var objNm  = "TB_INFLUX_PATH_REF_STAT";var statisDate = "20190312"; var statisType = "D"
...
/*
          ...
          INSERT INTO TB_INFLUX_PATH_REF_STAT(STATIS_DATE, VHOST, VISIT_PATH, ATTR_ID, REF_URL, VISIT_CNT, TOT_VISITOR_CNT, NEW_VISITOR_CNT, RE_VISITOR_CNT, LOGIN_CNT, LOGIN_VISITOR_CNT, DUR_TIME, PAGE_CNT, PAGE_VIEW, BOUNCE_CNT)
          WITH REFERER_VISIT_PATH AS
          (
          SELECT SESSION_ID, TRIM(CATEGORY) AS VISIT_PATH, HOST||DIR_CGI REF_URL
          ...
          SELECT  ? AS STATIS_DATE,
            ...
            SELECT  NVL(VHOST, 'TOTAL') AS VHOST, NVL(TRIM(CATEGORY), 'TOTAL') AS VISIT_PATH, 'ALL' AS ATTR_ID, HOST||DIR_CGI REF_URL,
            ...
            (
            -- 로그인 수, 로그인 회원수
            SELECT NVL(VHOST, 'TOTAL') AS VHOST, NVL(VISIT_PATH, 'TOTAL') AS VISIT_PATH, 'ALL' AS ATTR_ID, REF_URL,
              SUM(LOGIN_CNT) AS LOGIN_CNT, COUNT(UNIQUE L_ID) AS LOGIN_VISITOR_CNT
              FROM
              (
              SELECT VHOST, NVL(VISIT_PATH, 'N') AS VISIT_PATH, L_ID, LOGIN_CNT, REF_URL
                  FROM TB_MEMBER_CLASS_SEG_STAT TA, REFERER_VISIT_PATH TB
                  WHERE TA.SESSION_ID = TB.SESSION_ID(+)
              )
              GROUP BY ROLLUP (VHOST, VISIT_PATH, REF_URL)
            ) TB,
            (
            -- 페이지뷰, 체류시간
            SELECT NVL(VHOST, 'TOTAL') AS VHOST, NVL(VISIT_PATH, 'TOTAL') AS VISIT_PATH, 'ALL' AS ATTR_ID, REF_URL,
            ...
            FROM
            (
            SELECT  TA.VHOST, NVL(VISIT_PATH, 'N') AS VISIT_PATH, TA.SESSION_ID, REF_URL,
              COUNT(1) BOUNCE_VIEW,
            ...
            GROUP BY V_ID, TA.SESSION_ID, TA.VHOST, VISIT_PATH, REF_URL
            )
            GROUP BY ROLLUP (VHOST, VISIT_PATH, REF_URL)
            ) TD
            WHERE TA.VHOST = TB.VHOST(+) AND TA.VISIT_PATH = TB.VISIT_PATH(+) AND TA.ATTR_ID = TB.ATTR_ID(+) AND TA.REF_URL = TB.REF_URL(+)
            AND   TA.VHOST = TC.VHOST(+) AND TA.VISIT_PATH = TC.VISIT_PATH(+) AND TA.ATTR_ID = TC.ATTR_ID(+) AND TA.REF_URL = TC.REF_URL(+)
            AND   TA.VHOST = TD.VHOST(+) AND TA.VISIT_PATH = TD.VISIT_PATH(+) AND TA.ATTR_ID = TD.ATTR_ID(+) AND TA.REF_URL = TD.REF_URL(+)
 ...
 * */
  ...
  def excuteSql() = {
    ...
    sqlDf2.cache.createOrReplaceTempView("REFERER_CATEGORY_REF_URL"); sqlDf2.count()

    qry =
    s"""
      SELECT DISTINCT GVHOST, CATEGORY, REF_URL
      FROM
          (
          SELECT GVHOST
               , (CASE WHEN CATEGORY IS NULL OR CATEGORY = '' THEN 'N' ELSE CATEGORY END) AS CATEGORY
               , (CASE WHEN HOST||DIR_CGI IS NULL OR HOST||DIR_CGI = '' THEN 'N' ELSE HOST||DIR_CGI END)    AS REF_URL
          FROM   TB_REFERER_SESSION
          UNION
          SELECT GVHOST
               , (CASE WHEN CATEGORY IS NULL OR CATEGORY = '' THEN 'N' ELSE CATEGORY END) AS CATEGORY
    ...
    sqlDf3.cache.createOrReplaceTempView("REFERER_REF_URL_META"); sqlDf3.count()
    ...
    FROM
           (
           SELECT GVHOST, CATEGORY, REF_URL
           FROM REFERER_REF_URL_META
           ) TA
           LEFT OUTER JOIN
           (
           -- 로그인방문자수, 로그인수
           SELECT
                ...
                ,(CASE WHEN TB.REF_URL IS NULL OR TB.REF_URL = '' THEN 'N' ELSE TB.REF_URL END)    AS REF_URL
           ...
           (CASE WHEN TB.REF_URL IS NULL OR TB.REF_URL = '' THEN 'N' ELSE TB.REF_URL END)
           ) TB
           ON  TA.GVHOST = TB.GVHOST AND TA.CATEGORY = TB.CATEGORY AND TA.REF_URL = TB.REF_URL
           LEFT OUTER JOIN
           (
           -- 페이지뷰, 페이지수, 체류시간
           SELECT
           ...
           FROM   TB_ACCESS_SESSION2 TA
                  LEFT OUTER JOIN  REFERER_CATEGORY_REF_URL  TB
                  ON TA.GVHOST = TB.GVHOST AND TA.SESSION_ID = TB.SESSION_ID
           GROUP BY TA.GVHOST, (CASE WHEN TB.CATEGORY IS NULL OR TB.CATEGORY = '' THEN 'N' ELSE TB.CATEGORY END), 
           (CASE WHEN TB.REF_URL IS NULL OR TB.REF_URL = '' THEN 'N' ELSE TB.REF_URL END)
           ) TC
           ON  TA.GVHOST = TC.GVHOST AND TA.CATEGORY = TC.CATEGORY AND TA.REF_URL = TC.REF_URL
           LEFT OUTER JOIN
           (
           -- 이탈수
           SELECT
           ...
           ON  TA.GVHOST = TD.GVHOST AND TA.CATEGORY = TD.CATEGORY AND TA.REF_URL = TD.REF_URL
           LEFT OUTER JOIN
           (
           -- 방문자수,신규방문자수,재방문자수
           SELECT
           ...
           ON  TA.GVHOST = TE.GVHOST AND TA.CATEGORY = TE.CATEGORY AND TA.REF_URL = TE.REF_URL
           LEFT OUTER JOIN
           (
           -- 방문수
           SELECT
           ...
           ON  TA.GVHOST = TF.GVHOST AND TA.CATEGORY = TF.CATEGORY AND TA.REF_URL = TF.REF_URL
    GROUP BY TA.GVHOST, TA.CATEGORY, TA.REF_URL
    """
    //spark.sql(qry).take(100).foreach(println);
