 ...
 * 출    력 : TB_VISIT_CITY_STAT
 * 수정내역 :
 * 2019-01-23 | 피승현 | 최초작성
 */
object VisitCityStat {

  var spark: SparkSession = null
  var objNm  = "TB_VISIT_CITY_STAT"

  var statisDate = ""
  var statisType = ""
  //var objNm  = "TB_VISIT_CITY_STAT";var statisDate = "20190312"; var statisType = "D"
...
/*
                    INSERT  INTO TB_VISIT_CITY_STAT
                    ( STATIS_DATE, VHOST, COUNTRY_NM, CITY_NM, VISIT_CNT, TOT_VISITOR_CNT, NEW_VISITOR_CNT, RE_VISITOR_CNT, LOGIN_CNT, LOGIN_VISITOR_CNT, DUR_TIME, PAGE_CNT, PAGE_VIEW, BOUNCE_CNT )
                    SELECT ? STATIS_DATE,
                                 NVL(TA.VHOST, 'TOTAL') VHOST,
                                 NVL(TA.COUNTRY_NM, 'TOTAL') COUNTRY_NM,
                                 NVL(TA.CITY, 'TOTAL') CITY,
              ...
              SELECT NVL(COUNTRY_NM, 'TOTAL') AS COUNTRY_NM,
                  ...
                  SELECT  NVL(FN_COUNTRY_SEARCH_B(V_IP), '알수없음') AS COUNTRY_NM,
                    NVL(FN_CITY_SEARCH_B(V_IP), 'for-city') AS CITY,
                  ...
                  GROUP   BY V_HOST, FN_COUNTRY_SEARCH_B(V_IP), FN_CITY_SEARCH_B(V_IP)
                  )
                GROUP   BY ROLLUP((VHOST, COUNTRY_NM, CITY))
                                ) TA,
                                (
                                 --유입경로별 로그인수
                                SELECT   NVL(COUNTRY_NM, 'TOTAL') AS COUNTRY_NM,
                                ...
                                FROM    (
                                            SELECT  U_ID, SESSION_ID,
                                                    ...
                                                    NVL(FN_CITY_SEARCH_B(V_IP), 'for-city') AS CITY, V_HOST VHOST
                                            FROM    TB_WL_REFERER_SESSION
                                            ) TA,  TB_MEMBER_CLASS_DAY_STAT TB
                                WHERE   TA.SESSION_ID = TB.SESSION_ID
                                AND        TA.VHOST=  TB.VHOST
                                GROUP   BY ROLLUP((TA.VHOST, COUNTRY_NM, CITY))
                                ) TB,
                                (
                                --유입경로별 체류시간, 페이지뷰
                SELECT   NVL(COUNTRY_NM, 'TOTAL') AS COUNTRY_NM,
                ...
                FROM
                (
                SELECT TA.VHOST, TA.SESSION_ID, NVL(COUNTRY_NM, '알수없음') AS COUNTRY_NM, NVL(CITY, 'for-city') AS CITY, PAGE_VIEW, DUR_TIME
                FROM
                (
                ...
                GROUP BY ROLLUP ((VHOST, COUNTRY_NM, CITY))
                    ...
                    WHERE   TA.COUNTRY_NM = TB.COUNTRY_NM(+) AND TA.CITY = TB.CITY(+) AND TA.VHOST = TB.VHOST(+)
                    AND       TA.COUNTRY_NM = TC.COUNTRY_NM(+) AND TA.CITY = TC.CITY(+) AND TA.VHOST = TC.VHOST(+)
                    AND       TA.COUNTRY_NM = TD.COUNTRY_NM(+) AND TA.CITY = TD.CITY(+) AND TA.VHOST = TD.VHOST(+)
  ...
  def excuteSql() = {
    ...
    FROM
           ...
           SELECT TA.GVHOST
                ...
                , CASE WHEN TB.CITY IS NULL THEN 'ETC' ELSE TB.CITY END  AS CITY
                , SUM(VISIT_CNT)             AS VISIT_CNT
           FROM   (
           ...
           LEFT OUTER JOIN
                  (
                   SELECT IP, MIN(COUNTRY) AS COUNTRY, MIN(CITY) AS CITY
                    FROM TB_IP_INFO
                    GROUP BY IP
                  ) TB
           ON  TA.V_IP = TB.IP
           GROUP  BY TA.GVHOST, TB.COUNTRY, TB.CITY
           ) TA
           LEFT OUTER JOIN
           (
           -- 로그인방문자수, 로그인수
           ...
           AND    TA.SESSION_ID = TB.SESSION_ID
           GROUP BY TA.GVHOST, TA.COUNTRY, TA.CITY
           ) TB
           ON  TA.GVHOST = TB.GVHOST AND TA.COUNTRY = TB.COUNTRY AND TA.CITY = TB.CITY
           LEFT OUTER JOIN
           (
           -- 페이지뷰, 페이지수, 체류시간
           SELECT
           ...
           ON  TA.GVHOST = TC.GVHOST AND TA.COUNTRY = TC.COUNTRY AND TA.CITY = TC.CITY
           LEFT OUTER JOIN
           (
           -- 이탈수
           SELECT
           ...
           ON  TA.GVHOST = TD.GVHOST AND TA.COUNTRY = TD.COUNTRY AND TA.CITY = TD.CITY
           LEFT OUTER JOIN
           (
           -- 방문자수,신규방문자수,재방문자수
           SELECT TA.GVHOST
           ...
           ON  TA.GVHOST = TE.GVHOST AND TA.COUNTRY = TE.COUNTRY AND TA.CITY = TE.CITY
    GROUP BY TA.GVHOST, TA.COUNTRY, TA.CITY
    """
