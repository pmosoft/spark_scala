 ...
 * 출    력 : TB_INFLUX_DOMAIN_DETAIL_STAT
 * 수정내역 :
 * 2019-02-07 | 피승현 | 최초작성
 */
object InfluxDomainDetailStat {

  var spark: SparkSession = null
  var objNm  = "TB_INFLUX_DOMAIN_DETAIL_STAT"

  var statisDate = ""
  var statisType = ""
  //var objNm  = "TB_INFLUX_DOMAIN_DETAIL_STAT";var statisDate = "20190312"; var statisType = "D"
...
/*
          ...
          INSERT INTO TB_INFLUX_DOMAIN_DETAIL_STAT(STATIS_DATE, VHOST, VISIT_PATH, INFLUX_DOMAIN, CHILD_DOMAIN, ATTR_ID, VISIT_CNT, TOT_VISITOR_CNT, NEW_VISITOR_CNT, RE_VISITOR_CNT, LOGIN_CNT, LOGIN_VISITOR_CNT, DUR_TIME, PAGE_CNT, PAGE_VIEW, BOUNCE_CNT)
          WITH REFERER_VISIT_PATH AS
          (
          SELECT SESSION_ID, TRIM(CATEGORY) AS VISIT_PATH, DOMAIN AS INFLUX_DOMAIN, HOST AS CHILD_DOMAIN
          ...
          SELECT  ? AS STATIS_DATE,
            ...
            FROM
            (
            SELECT  NVL(VHOST, 'TOTAL') AS VHOST, NVL(TRIM(CATEGORY), 'TOTAL') AS VISIT_PATH, NVL(DOMAIN, 'TOTAL') AS INFLUX_DOMAIN, NVL(HOST, 'TOTAL') AS CHILD_DOMAIN, 'ALL' AS ATTR_ID,
            ...
            FROM    TB_WL_REFERER_SEGMENT
            GROUP   BY  ROLLUP ((VHOST, TRIM(CATEGORY), DOMAIN, HOST))
            ) TA,
            (
            SELECT NVL(VHOST, 'TOTAL') AS VHOST, NVL(VISIT_PATH, 'TOTAL') AS VISIT_PATH, NVL(INFLUX_DOMAIN, 'TOTAL') AS INFLUX_DOMAIN, NVL(CHILD_DOMAIN, 'TOTAL') AS CHILD_DOMAIN, 'ALL' AS ATTR_ID,
              SUM(LOGIN_CNT) AS LOGIN_CNT, COUNT(UNIQUE L_ID) AS LOGIN_VISITOR_CNT
              FROM
              (
              SELECT VHOST, NVL(VISIT_PATH, 'N') AS VISIT_PATH, NVL(INFLUX_DOMAIN, 'N') AS INFLUX_DOMAIN, NVL(CHILD_DOMAIN, 'N') AS CHILD_DOMAIN, L_ID, LOGIN_CNT
                  FROM TB_MEMBER_CLASS_SEG_STAT TA, REFERER_VISIT_PATH TB
                  WHERE TA.SESSION_ID = TB.SESSION_ID(+)
              )
              GROUP BY ROLLUP ((VHOST, VISIT_PATH, INFLUX_DOMAIN, CHILD_DOMAIN))
            ) TB,
            (
            SELECT NVL(VHOST, 'TOTAL') AS VHOST, NVL(VISIT_PATH, 'TOTAL') AS VISIT_PATH, NVL(INFLUX_DOMAIN, 'TOTAL') AS INFLUX_DOMAIN, NVL(CHILD_DOMAIN, 'TOTAL') AS CHILD_DOMAIN, 'ALL' AS ATTR_ID,
              ...
              FROM
              (
              SELECT  VHOST, NVL(VISIT_PATH, 'N') AS VISIT_PATH, NVL(INFLUX_DOMAIN, 'N') AS INFLUX_DOMAIN, NVL(CHILD_DOMAIN, 'N') AS CHILD_DOMAIN,
                  PAGE_VIEW, DUR_TIME
            ...
            FROM
            (
            SELECT  TA.VHOST, NVL(VISIT_PATH, 'N') AS VISIT_PATH, NVL(INFLUX_DOMAIN, 'N') AS INFLUX_DOMAIN, NVL(CHILD_DOMAIN, 'N') AS CHILD_DOMAIN, TA.SESSION_ID,
              COUNT(1) BOUNCE_VIEW,
            ...
            GROUP BY V_ID, TA.SESSION_ID, TA.VHOST, VISIT_PATH, NVL(INFLUX_DOMAIN, 'N'), NVL(CHILD_DOMAIN, 'N')
            )
            GROUP BY ROLLUP ((VHOST, VISIT_PATH, INFLUX_DOMAIN, CHILD_DOMAIN))
            ) TD
            WHERE TA.VHOST = TB.VHOST(+) AND TA.VISIT_PATH = TB.VISIT_PATH(+) AND TA.INFLUX_DOMAIN = TB.INFLUX_DOMAIN(+) AND TA.CHILD_DOMAIN = TB.CHILD_DOMAIN(+) AND TA.ATTR_ID = TB.ATTR_ID(+)
            AND   TA.VHOST = TC.VHOST(+) AND TA.VISIT_PATH = TC.VISIT_PATH(+) AND TA.INFLUX_DOMAIN = TC.INFLUX_DOMAIN(+) AND TA.CHILD_DOMAIN = TC.CHILD_DOMAIN(+)  AND TA.ATTR_ID = TC.ATTR_ID(+)
            AND   TA.VHOST = TD.VHOST(+) AND TA.VISIT_PATH = TD.VISIT_PATH(+) AND TA.INFLUX_DOMAIN = TD.INFLUX_DOMAIN(+) AND TA.CHILD_DOMAIN = TD.CHILD_DOMAIN(+)  AND TA.ATTR_ID = TD.ATTR_ID(+)
 ...
 * */
  ...
  def excuteSql() = {
    ...
    sqlDf2.cache.createOrReplaceTempView("REFERER_CATEGORY_DOMAIN_HOST"); sqlDf2.count()

    qry =
    s"""
      SELECT DISTINCT GVHOST, CATEGORY, DOMAIN, HOST
      FROM
          (
          SELECT GVHOST
    ...
    sqlDf3.cache.createOrReplaceTempView("REFERER_DOMAIN_HOST_META"); sqlDf3.count()
    ...
    FROM
           (
           SELECT GVHOST, CATEGORY, DOMAIN, HOST
           FROM REFERER_DOMAIN_HOST_META
           ) TA
           LEFT OUTER JOIN
           (
           -- 로그인방문자수, 로그인수
           ...
           ON  TA.GVHOST = TB.GVHOST AND TA.CATEGORY = TB.CATEGORY AND TA.DOMAIN = TB.DOMAIN AND TA.HOST = TB.HOST
           LEFT OUTER JOIN
           (
           -- 페이지뷰, 페이지수, 체류시간, 이탈수
           SELECT
           ...
           FROM   TB_ACCESS_SESSION2 TA
                  LEFT OUTER JOIN  REFERER_CATEGORY_DOMAIN_HOST  TB
                  ON TA.GVHOST = TB.GVHOST AND TA.SESSION_ID = TB.SESSION_ID
           GROUP BY TA.GVHOST, (CASE WHEN TB.CATEGORY IS NULL OR TB.CATEGORY = '' THEN 'N' ELSE TB.CATEGORY END), 
           (CASE WHEN TB.DOMAIN IS NULL OR TB.DOMAIN = '' THEN 'N' ELSE TB.DOMAIN END), 
           (CASE WHEN TB.HOST IS NULL   OR TB.HOST = ''   THEN 'N' ELSE TB.HOST   END)
           ) TC
           ON  TA.GVHOST = TC.GVHOST AND TA.CATEGORY = TC.CATEGORY AND TA.DOMAIN = TC.DOMAIN AND TA.HOST = TC.HOST
           LEFT OUTER JOIN
           (
           -- 이탈수
           SELECT
           ...
           ON  TA.GVHOST = TD.GVHOST AND TA.CATEGORY = TD.CATEGORY AND TA.DOMAIN = TD.DOMAIN AND TA.HOST = TD.HOST
           LEFT OUTER JOIN
           (
           -- 방문자수,신규방문자수,재방문자수
           SELECT
           ...
           ON  TA.GVHOST = TE.GVHOST AND TA.CATEGORY = TE.CATEGORY AND TA.DOMAIN = TE.DOMAIN AND TA.HOST = TE.HOST
           LEFT OUTER JOIN
           (
           -- 방문수
           SELECT
           ...
           ON  TA.GVHOST = TF.GVHOST AND TA.CATEGORY = TF.CATEGORY AND TA.DOMAIN = TF.DOMAIN AND TA.HOST = TF.HOST
    GROUP BY TA.GVHOST, TA.CATEGORY, TA.DOMAIN, TA.HOST
    """
    //spark.sql(qry).take(100).foreach(println);
