 ...
 * 출    력 : TB_VISIT_OS_VER_STAT
 * 수정내역 :
 * 2019-01-24 | 피승현 | 최초작성
 */
object VisitOsVerStat {

  var spark: SparkSession = null
  var objNm  = "TB_VISIT_OS_VER_STAT"

  var statisDate = ""
  var statisType = ""
  //var objNm  = "TB_VISIT_OS_VER_STAT"; var statisDate = "20190312"; var statisType = "D"
...
/*
                   ...
                   INSERT  INTO TB_VISIT_OS_VER_STAT
                  ( STATIS_DATE, VHOST, OS, VERSION, VISIT_CNT, TOT_VISITOR_CNT, NEW_VISITOR_CNT, RE_VISITOR_CNT, LOGIN_CNT, LOGIN_VISITOR_CNT, DUR_TIME, PAGE_CNT, PAGE_VIEW, BOUNCE_CNT )
                     SELECT ? STATIS_DATE,
                                 NVL(TA.VHOST, 'TOTAL') VHOST,
                                 NVL(TA.OS, 'TOTAL') OS,
                                 NVL(TA.OS_VER, 'TOTAL') OS_VER,
                    ...
                    FROM    (
                                ...
                                GROUP   BY ROLLUP((V_HOST, OS, OS_VER))
                                ) TA,
                                (
                                 --유입경로별 로그인수
                                SELECT   NVL(OS, 'TOTAL') AS OS,
                ...
                FROM
                (
                SELECT TA.VHOST, TA.SESSION_ID, NVL(OS, 'ETC') AS OS, NVL(OS_VER, 'ETC') AS OS_VER, PAGE_VIEW, DUR_TIME
                FROM
                (
                ...
                (
                SELECT  V_HOST VHOST, SESSION_ID, OS, OS_VER
                FROM    TB_WL_REFERER_SESSION
                ) TB
                WHERE TA.VHOST = TB.VHOST(+)
                AND   TA.SESSION_ID = TB.SESSION_ID(+)
                )
                GROUP BY ROLLUP (VHOST, OS, OS_VER)
                                ) TC,
                                (
                                --유입경로별 이탈수
                                SELECT  NVL(OS, 'TOTAL') AS OS,
                    ...
                    WHERE   TA.OS = TB.OS(+) AND TA.OS_VER = TB.OS_VER(+) AND TA.VHOST = TB.VHOST(+)
                    AND       TA.OS = TC.OS(+) AND TA.OS_VER = TC.OS_VER(+) AND TA.VHOST = TC.VHOST(+)
                    AND       TA.OS = TD.OS(+) AND TA.OS_VER = TD.OS_VER(+) AND TA.VHOST = TD.VHOST(+)
 ...
 * */
  ...
  def excuteSql() = {
    ...
    qry =
    s"""
      SELECT DISTINCT GVHOST, OS, OS_VER  
             FROM
             (
             SELECT GVHOST
                  , (CASE WHEN OS IS NULL OR OS = '' THEN 'NA' ELSE OS END) AS OS
                  , (CASE WHEN OS_VER IS NULL OR OS_VER = '' THEN 'NA' ELSE OS_VER END)  AS OS_VER
    ...
    sqlDf2.cache.createOrReplaceTempView("ALL_OS_VERSION"); sqlDf2.count()
    ...
    FROM
           ...
           SELECT
                ...
                ,(CASE WHEN OS_VER IS NULL OR OS_VER = '' THEN 'NA' ELSE OS_VER END) OS_VER
           ...
           AND    TA.SESSION_ID = TB.SESSION_ID
           GROUP BY TA.GVHOST, TA.OS, TA.OS_VER
           ) TB
           ON  TA.GVHOST = TB.GVHOST AND TA.OS = TB.OS AND TA.OS_VER = TB.OS_VER
           LEFT OUTER JOIN
           (
           -- 페이지뷰, 페이지수, 체류시간
           SELECT
           ...
           FROM   TB_ACCESS_SESSION2
           GROUP BY GVHOST, OS, OS_VER
           ) TC
           ON  TA.GVHOST = TC.GVHOST AND TA.OS = TC.OS AND TA.OS_VER = TC.OS_VER
           LEFT OUTER JOIN
           (
           -- 이탈수
           SELECT
           ...
           ON  TA.GVHOST = TD.GVHOST AND TA.OS = TD.OS AND TA.OS_VER = TD.OS_VER
           LEFT OUTER JOIN
           (
           -- 방문자수,신규방문자수,재방문자수
           SELECT
           ...
           ON  TA.GVHOST = TE.GVHOST AND TA.OS = TE.OS AND TA.OS_VER = TE.OS_VER
           LEFT OUTER JOIN
           (
           -- 방문수
           SELECT
           ...
           ON  TA.GVHOST = TF.GVHOST AND TA.OS = TF.OS AND TA.OS_VER = TF.OS_VER
    GROUP BY TA.GVHOST, TA.OS, TA.OS_VER
    """
    //spark.sql(qry).take(100).foreach(println);
