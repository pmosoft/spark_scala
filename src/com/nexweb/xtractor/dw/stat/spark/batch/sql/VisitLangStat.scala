 ...
 * 출    력 : TB_VISIT_LANG_STAT
 * 수정내역 :
 * 2019-01-23 | 피승현 | 최초작성
 */
object VisitLangStat {

  var spark: SparkSession = null
  var objNm  = "TB_VISIT_LANG_STAT"

  var statisDate = ""
  var statisType = ""
  //var objNm  = "TB_VISIT_LANG_STAT"; var statisDate = "20190312"; var statisType = "D"
...
/*
                     ...
                     INSERT  INTO TB_VISIT_LANG_STAT
                    ( STATIS_DATE, VHOST, LANGUAGE, VISIT_CNT, TOT_VISITOR_CNT, NEW_VISITOR_CNT, RE_VISITOR_CNT, LOGIN_CNT, LOGIN_VISITOR_CNT, DUR_TIME, PAGE_CNT, PAGE_VIEW, BOUNCE_CNT )
                     SELECT ? STATIS_DATE,
                                 NVL(TA.VHOST, 'TOTAL') VHOST,
                                 NVL(TA.LANG, 'TOTAL') LANG,
                    ...
                    WHERE   TA.LANG = TB.LANG(+) AND TA.VHOST = TB.VHOST(+)
                    AND       TA.LANG = TC.LANG(+) AND TA.VHOST = TC.VHOST(+)
                    AND       TA.LANG = TD.LANG(+) AND TA.VHOST = TD.VHOST(+)
 ...
 * */
  ...
  def excuteSql() = {
    ...
    qry =
    s"""
      SELECT DISTINCT GVHOST, LANG  
             FROM
             (
             SELECT GVHOST
                  , (CASE WHEN LANG IS NULL OR LANG = '' THEN 'NA' ELSE LANG END) AS LANG
    ...
    sqlDf2.cache.createOrReplaceTempView("ALL_LANG"); sqlDf2.count()
    ...
    FROM
           ...
           SELECT
                  TA.GVHOST                                                        AS GVHOST
                ,(CASE WHEN LANG IS NULL OR LANG = '' THEN 'NA' ELSE LANG END) LANG
           ...
           GROUP BY TA.GVHOST, TA.LANG
           ) TB
           ON  TA.GVHOST = TB.GVHOST AND TA.LANG = TB.LANG
           LEFT OUTER JOIN
           (
           -- 페이지뷰, 페이지수, 체류시간
           SELECT
           ...
           FROM   TB_ACCESS_SESSION2
           GROUP BY GVHOST, LANG
           ) TC
           ON  TA.GVHOST = TC.GVHOST AND TA.LANG = TC.LANG
           LEFT OUTER JOIN
           (
           -- 이탈수
           SELECT
           ...
           ON  TA.GVHOST = TD.GVHOST AND TA.LANG = TD.LANG
           LEFT OUTER JOIN
           (
           -- 방문자수, 신규방문자수, 재방문자수
           SELECT
           ...
           ON  TA.GVHOST = TE.GVHOST AND TA.LANG = TE.LANG
           LEFT OUTER JOIN
           (
           -- 방문수
           SELECT
           ...
           ON  TA.GVHOST = TF.GVHOST AND TA.LANG = TF.LANG
    GROUP BY TA.GVHOST, TA.LANG
    """
    //spark.sql(qry).take(100).foreach(println);
