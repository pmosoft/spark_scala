 ...
 * 출    력 : TB_VISIT_BROWSER_STAT
 * 수정내역 :
 * 2019-01-23 | 피승현 | 최초작성
 */
object VisitBrowserStat {

  var spark: SparkSession = null
  var objNm  = "TB_VISIT_BROWSER_STAT"

  var statisDate = ""
  var statisType = ""
  //var objNm  = "TB_VISIT_BROWSER_STAT"; var statisDate = "20190312"; var statisType = "D"
...
/*
                   ...
                   INSERT  INTO TB_VISIT_BROWSER_STAT
                  ( STATIS_DATE, VHOST, BROWSER, VISIT_CNT, TOT_VISITOR_CNT, NEW_VISITOR_CNT, RE_VISITOR_CNT, LOGIN_CNT, LOGIN_VISITOR_CNT, DUR_TIME, PAGE_CNT, PAGE_VIEW, BOUNCE_CNT )
                   SELECT ? STATIS_DATE,
                  ...
                  FROM    (
                              ...
                              GROUP   BY ROLLUP(V_HOST, BROWSER)
                              ) TA,
                              (
                               --유입경로별 로그인수
                              SELECT   NVL(BROWSER, 'TOTAL') AS BROWSER,
                ...
                FROM
                (
                SELECT TA.VHOST, TA.SESSION_ID, NVL(BROWSER, 'ETC') AS BROWSER, PAGE_VIEW, DUR_TIME
                FROM
                (
                ...
                (
                SELECT  V_HOST VHOST, SESSION_ID, BROWSER
                FROM    TB_WL_REFERER_SESSION
                ) TB
                WHERE TA.VHOST = TB.VHOST(+)
                AND   TA.SESSION_ID = TB.SESSION_ID(+)
                )
                GROUP BY ROLLUP (VHOST, BROWSER)
                              ) TC,
                              (
                              --유입경로별 이탈수
                              SELECT  NVL(BROWSER, 'TOTAL') AS BROWSER,
                  ...
                  WHERE   TA.BROWSER = TB.BROWSER(+) AND TA.VHOST = TB.VHOST(+)
                  AND       TA.BROWSER = TC.BROWSER(+) AND TA.VHOST = TC.VHOST(+)
                  AND       TA.BROWSER = TD.BROWSER(+) AND TA.VHOST = TD.VHOST(+)
 ...
 * */
  ...
  def excuteSql() = {
    ...
    qry =
    s"""
      SELECT DISTINCT GVHOST, BROWSER  
             FROM
             (
             SELECT GVHOST
    ...
    sqlDf2.cache.createOrReplaceTempView("ALL_BROWSER"); sqlDf2.count()
    ...
    FROM
           ...
           AND    TA.SESSION_ID = TB.SESSION_ID
           GROUP BY TA.GVHOST, TA.BROWSER
           ) TB
           ON  TA.GVHOST = TB.GVHOST AND TA.BROWSER = TB.BROWSER
           LEFT OUTER JOIN
           (
           -- 페이지뷰, 페이지수, 체류시간
           SELECT
           ...
           ON  TA.GVHOST = TC.GVHOST AND TA.BROWSER = TC.BROWSER
           LEFT OUTER JOIN
           (
           -- 이탈수
           SELECT
           ...
           ON  TA.GVHOST = TD.GVHOST AND TA.BROWSER = TD.BROWSER
           LEFT OUTER JOIN
           (
           -- 방문자수,신규방문자수,재방문자수
           SELECT
           ...
           ON  TA.GVHOST = TE.GVHOST AND TA.BROWSER = TE.BROWSER
           LEFT OUTER JOIN
           (
           -- 방문수
           SELECT
           ...
           ON  TA.GVHOST = TF.GVHOST AND TA.BROWSER = TF.BROWSER
    GROUP BY TA.GVHOST, TA.BROWSER
    """
    //spark.sql(qry).take(100).foreach(println);
