...
object VisitIspStat {
  ...
  var objNm  = "TB_VISIT_ISP_STAT"

  var statisDate = ""
  var statisType = ""
  //var objNm  = "TB_VISIT_ISP_STAT";var statisDate = "20190313"; var statisType = "D"
...
/*
          ...
          INSERT  INTO TB_VISIT_ISP_STAT
                    ( STATIS_DATE, VHOST, ISP_NM, VISIT_CNT, TOT_VISITOR_CNT, NEW_VISITOR_CNT, RE_VISITOR_CNT, LOGIN_CNT, LOGIN_VISITOR_CNT, DUR_TIME, PAGE_CNT, PAGE_VIEW, BOUNCE_CNT )
                     SELECT ? STATIS_DATE,
                                 NVL(TA.VHOST, 'TOTAL') VHOST,
                                 NVL(TA.ISP_NM, 'TOTAL') ISP_NM,
                    ...
                    FROM    (
                                --유입경로별 방문수
                                SELECT  NVL(ISP_NM, 'TOTAL') AS ISP_NM,
                                ...
                                FROM    (
                SELECT  NVL(FN_ISP_SEARCH_B(V_IP), '알수없음') AS ISP_NM,
                  V_HOST, V_ID
                FROM    TB_WL_REFERER_SESSION
                )
                                GROUP   BY ROLLUP(V_HOST, ISP_NM)
                                ) TA,
                                (
                                 --유입경로별 로그인수
                                SELECT   NVL(ISP_NM, 'TOTAL') AS ISP_NM,
                ...
                FROM
                (
                SELECT TA.VHOST, TA.SESSION_ID, NVL(ISP_NM, '알수없음') AS ISP_NM, PAGE_VIEW, DUR_TIME
                FROM
                (
                ...
                GROUP BY ROLLUP (VHOST, ISP_NM)
                                ...
                                SELECT  NVL(ISP_NM, 'TOTAL') AS ISP_NM,
                                    ...
                                    FROM    (
                                            SELECT  SESSION_ID, V_HOST VHOST,
                                                    NVL(FN_ISP_SEARCH_B(V_IP), '알수없음') AS ISP_NM
                                            FROM    TB_WL_REFERER_SESSION
                                            ) TA,
                                            (
                    ...
                    WHERE   TA.ISP_NM = TB.ISP_NM(+) AND TA.VHOST = TB.VHOST(+)
                    AND       TA.ISP_NM = TC.ISP_NM(+) AND TA.VHOST = TC.VHOST(+)
                    AND       TA.ISP_NM = TD.ISP_NM(+) AND TA.VHOST = TD.VHOST(+)
 ...
 * */
  ...
  def excuteSql() = {
    ...
    FROM
           ...
           -- 방문수
           SELECT TA.GVHOST
                , CASE WHEN TB.ISP_NM IS NULL THEN 'ETC' ELSE TB.ISP_NM END  AS ISP_NM
           ...
           LEFT OUTER JOIN
                  (
                   SELECT IP, MIN(ISP_NM) AS ISP_NM
                    FROM TB_IP_INFO
                    GROUP BY IP
                  ) TB
           ON  TA.V_IP = TB.IP
           GROUP  BY TA.GVHOST, TB.ISP_NM
           ) TA
           LEFT OUTER JOIN
           (
           -- 로그인방문자수, 로그인수
           ...
           AND    TA.SESSION_ID = TB.SESSION_ID
           GROUP BY TA.GVHOST, TA.ISP_NM
           ) TB
           ON  TA.GVHOST = TB.GVHOST AND TA.ISP_NM = TB.ISP_NM
           LEFT OUTER JOIN
           (
           -- 페이지뷰, 페이지수, 체류시간
           SELECT
           ...
           ON  TA.GVHOST = TC.GVHOST AND TA.ISP_NM = TC.ISP_NM
           LEFT OUTER JOIN
           (
           -- 이탈수
           SELECT
           ...
           ON  TA.GVHOST = TD.GVHOST AND TA.ISP_NM = TD.ISP_NM
           LEFT OUTER JOIN
           (
           ...
           ON  TA.GVHOST = TE.GVHOST AND TA.ISP_NM = TE.ISP_NM
    GROUP BY TA.GVHOST, TA.ISP_NM
    """
