/**
 *******************************************************************************
 * @brief  JOB SCHEDULE에 등록된 SCENARIO를 한건씩 읽어 분석서버에 요청한다.
 * @remark
 * @code
 * Usage ScheduleDaemon {start|stop}
 * @endcode
 * @file   ScheduleDaemon.c
 * @date   2012.12.01
 * @author Ysic
 *******************************************************************************
 */
...
#ifdef __EX
#define CONFIG_FILE     	"./conf/ScheduleDaemon_Ex.conf"   // 설정파일명
#elif __SWING
#define CONFIG_FILE     	"./conf/ScheduleDaemon_Swing.conf"   // 설정파일명
#elif __NEWSITE
#define CONFIG_FILE     	"./conf/ScheduleDaemon_NewSite.conf"   // 설정파일명
#else
#define CONFIG_FILE     	"./conf/ScheduleDaemon.conf"   // 설정파일명
#endif
...
int main(int argc, char* argv[])
{
    ...
    log_info(__FFILE__, __FFUNC__, __LINE__, "Log Info: db_user(%s), db_pwd(%s), db_sid(%s)"
    			, cfg_getString("db_user")
                ,cfg_getString("db_pwd")
                ,cfg_getString("db_sid"));
...
static void stopProcess(void)
{
    ...
    printf("SCHEDULE데몬종료시그널을 보냈습니다. 프로세스를 확인하세요.[%d] \n", PID);
    kill(PID, SIGTERM);
}
    ...
    while(s_daemonOnOffFlag) {
        ...
        if(db_testCall() != SQLSUCCESS) {
            ...
            if(ret != SQLSUCCESS) {
                ...
                // 관리자 SMS전송
                sms_sendSystemManager("ScheduleDaemon DB연결 오류 발생. ORA[%d]", ret);
        ...
        // schedule 처리 로직 호출
        ret = procSchedule(&st_server, &st_analysis_weight);
